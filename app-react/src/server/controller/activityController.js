...
module.exports = {
    ...
    update: async (req, res) => {
        let obj = new Activity(req.body);
        await Activity.updateOne({ _id: obj._id }, obj, function (err) {
            (err ? res.status(400).send(err) : res.status(200).json(obj));
        });
    },
    ...
    filtrate: async (req, res) => {
        const objects = await Activity.find({
            $or: [
